...
#genetic drift, mutation and migration all t-1 generation to t instead of
#genetic drift (t-1 -> t), mutation (t->t), migration (t->t)
 ...
 for t in 2:T+1 #Generations #T+1
    # println("generation######################",t)
     for i in 1:nDemes
        ...
        #but from last generation t-1
        # xwt = x[end,t-1] #wild type frequency at t-1 #defined before

        m = pois_rand(mu*xwt*NDeme) #number of de novo mutants generated
        # println("mutation",m)
    ...
    for i in 1:nDemes
        ...
        MIG = mig*x_B*NDeme #expected number of migrants individuals #column vec
        # println("MIG",MIG)
        ...
        #in case of irational migration rate
        if sum(migLV) > NDeme #can't migrate more than deme population size
            migLV = round.(migLV/sum(migLV)*NDeme) #could lead the sum less than NDeme
        end
